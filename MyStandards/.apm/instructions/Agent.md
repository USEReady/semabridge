...
This agent instruction must be used together with the following files in the same folder:

- `Instructions.md` — Core architecture, coding standards, and implementation constraints.
- `Prompt.md` — Operational mode prompts for cleanup and code validation workflows.

If any directive in these files appears to conflict, prioritize the stricter rule and preserve runtime safety.
