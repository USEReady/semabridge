...
python tests/test_measure_pipeline_detailed.py --workspace-id 12345-abcde --dataset COMPETITIVE_MARKETING_ANALYSIS

# Custom output location
python tests/test_measure_pipeline_detailed.py --dataset COMPETITIVE_MARKETING_ANALYSIS --output ./debug_reports/my_analysis
```
...
**What it does:**
1. ✅ Extracts all measures directly from Fabric TMSL
2. ✅ Converts them through the tmsl_to_sml transformer
3. ✅ Checks which measures are eligible for Snowflake generation
4. ✅ Generates detailed comparison report showing:
   - Complete measures (all 3 stages: Fabric → SML → Snowflake)
...
Stage Counts:
  ...
  • SML (canonical):         45 measures
  • Snowflake (target):      43 measures
...
❌ Fabric → SML Dropoff (2 measures):
  ...
  • competitive_marketing_analysis.Hidden Measure 2
    Why: Likely filtered by tmsl_to_sml converter (hidden, system table, etc)

❌ SML → Snowflake Dropoff (2 measures):
  • competitive_marketing_analysis.Complex Time Measure
    Reason: Complex DAX expression requires decomposition
    Complexity: Tier 3
    Time Intelligence: True
  
  • competitive_marketing_analysis.Conditional Calc
...
**Usage:**
```bash
python tests/test_measure_pipeline.py --workspace-id WS_ID --dataset DATASET_NAME --output markdown
```
...
- ✅ All measures from Fabric TMSL
- ❌ Hidden measures (if they exist)
- ❌ System tables (auto-skipped)

### Stage 2: SML Conversion
- ✅ Measures that made it through tmsl_to_sml
- ❌ Filtered measures (why they were dropped)
- Transformation details (expression → SQL)
- Complexity tier classification

### Stage 3: Snowflake SQL Generation
- ✅ Measures eligible for METRICS clause
- ❌ Measures that failed to convert (reasons)
...
# Run with Python
python tests/test_measure_pipeline_detailed.py --dataset "My Dataset Name"
```
...
LOGLEVEL=DEBUG python tests/test_measure_pipeline_detailed.py --dataset "My Dataset Name"
```
...
# 2. Look for "SML → Snowflake Dropoff" section
# 3. Check the "Reason" column for each dropped measure
# 4. Based on reason:

# If "Complexity: Tier 3" or "Time Intelligence: True"
#   → Need to use performance aggregation or decompose the measure

# If "Custom expression"
#   → May need manual conversion to SQL

# If "Depends on other measures"
#   → Ensure dependent measures are synced first
```
