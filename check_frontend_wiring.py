#!/usr/bin/env python3
"""
Comprehensive Frontend Wiring Check
...
expected_endpoints = {
    ...
    '/api/models': ['GET'],
    '/api/models/{model_id}': ['GET', 'PUT', 'DELETE'],
    '/api/repo/tree': ['GET'],
    '/api/repo/models': ['GET'],
    '/api/history': ['GET'],
}
        ...
        if path.rstrip('/') == endpoint.rstrip('/'):
            ...
            for method in methods:
                if method in actual_methods:
                    ...
                    print(f"  [XX] {method.ljust(6)} {path} (method missing)")
                    missing.append((endpoint, method))
            break
    
    if not found:
        print(f"  [XX] {endpoint} (ENDPOINT MISSING)")
        missing.append((endpoint, 'ALL'))

print(f"\nTotal Endpoints: {len(routes_by_path)}")
...
for name, path in contexts.items():
    with open(path) as f:
        ...
        status = "[OK]" if (has_provider and has_hook) else "[XX]"
        print(f"  {status} {name}")
...
with open("frontend/src/App.jsx") as f:
    ...
    print(f"  {'[OK]' if has_workspace_provider else '[XX]'} WorkspaceProvider")
    print(f"  {'[OK]' if has_logs_provider else '[XX]'} LogsProvider")
...
with open("frontend/src/App.jsx") as f:
    app = f.read()
    print("  [OK] SourceBrowser receives selectedItems" if "selectedItems={selectedItems}" in app else "  [XX] SourceBrowser missing selectedItems")
    print("  [OK] YamlEditor receives selectedItems" if "selectedItems={selectedItems}" in app else "  [XX] YamlEditor missing selectedItems")
    print("  [OK] YamlEditor receives sourceType" if "sourceType={sourceType}" in app else "  [XX] YamlEditor missing sourceType")
