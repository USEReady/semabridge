# Formats folder - convert fabric to supported structure in SF

Status: In Progress
Priority: High
Time: Time-intensive
assigned on : January 21, 2026

Formats folder: 

for each and every source and target connector supported by semabridge, store a list of the naming conventions and rules followed by that particular connector, so when a sync is done from a source to target, the necessary rules can be referred to, implemented and followed in code. 

include regex like checks to validate the name format and conversion functions to make a particular erroneous input abide by the rules. please give documentation for this too. 

Make sure that this includes metadata-specific formats for each connector (i.e snowflake, databricks, powerbi/fabric)

For eg Date format in one system might be different in another, so all this information needs to be cleaarly stored. Any other such variations should be captured along with their validation and conversion mechanism. Remember, this is only needed for the metadata layer, as we are doing a semantic sync.  

---

---

---

# Semabridge: Formats, Naming & Metadata Standards

**Scope:**Â Identifier validation, reserved keyword management, andÂ **metadata value formatting**Â (dates, literals, types) for the semantic layer.

## 1. Core Requirements

- **REQ-FMT-001 (Connector Specificity):**
    - Every supported connector (Snowflake, Databricks, PowerBI/Fabric) must have a dedicated "Format Definition" in theÂ `formats/`Â directory.
    - This definition serves as the single source of truth for all syntax rules required to generate valid metadata (DDL, JSON, TMSL) for that system.
- **REQ-FMT-002 (Identifier Validation & Sanitization):**
    - **Validation:**Â A functionÂ `is_valid_identifier(name)`Â using Regex and keyword lists.
    - **Sanitization:**Â A functionÂ `sanitize_identifier(name)`Â that automatically fixes non-compliant names (e.g., converting spaces to underscores for Snowflake, handling casing).
- **REQ-FMT-003 (Metadata Value Formatting):**
    - The module must define rules for formattingÂ **literals and default values**Â found in semantic metadata.
    - *Example:*Â A semantic model defines a default filter onÂ `OrderDate = '2023-01-01'`.
        - **Snowflake:**Â RequiresÂ `'2023-01-01'::DATE`.
        - **Power BI (DAX):**Â RequiresÂ `DATE(2023, 1, 1)`.
        - **Databricks:**Â RequiresÂ `DATE '2023-01-01'`.
    - The system must validate and convert these literals during theÂ osi/sml `-> Target`Â phase.
    - The is_valid_type(); sanitize_type() must be available for metadata value literals as well.
- **REQ-FMT-004 (Type Mapping Registry):**
    - Each format must provide a mapping fromÂ **SML Standard Types**Â (String, Integer, Float, Date, Boolean) to theÂ **System Specific Types**Â used in their metadata definitions.

---

## 2. Directory Structure

The structure now explicitly includes Databricks and separates Naming from Value formatting logic if complex. Make the structure and form of implementation extensible so more formats can be added easily with time. 

Plaintext

`semabridge/
â”œâ”€â”€ src/
â”‚   â””â”€â”€ semabridge/
â”‚       â”œâ”€â”€ formats/
â”‚       â”‚   â”œâ”€â”€ __init__.py           # Registry
â”‚       â”‚   â”œâ”€â”€ base_format.py        # Abstract Base Class
â”‚       â”‚   â”œâ”€â”€ snowflake_fmt.py      # Snowflake implementation
â”‚       â”‚   â”œâ”€â”€ fabric_fmt.py         # PowerBI/Fabric implementation
â”‚       â”‚   â””â”€â”€ databricks_fmt.py     # Databricks implementation
â”‚       â””â”€â”€ ...`

---

## 3. Implementation Standards

### 3.1 The Expanded Interface (`base_format.py`)

This interface now handles bothÂ *Names*Â (Identifiers) andÂ *Values*Â (Metadata Literals).

uv run `from abc import ABC, abstractmethod
from typing import List, Any
import re

class BaseFormatDefinition(ABC):
    """
    Abstract contract for system-specific syntax rules (Identifiers & Data).
    """
    
    # --- Section A: Naming & Identifiers ---
    
    @property
    @abstractmethod
    def max_identifier_length(self) -> int:
        pass

    @abstractmethod
    def is_valid_identifier(self, name: str) -> bool:
        """Returns True if the name meets regex/keyword rules."""
        pass

    @abstractmethod
    def sanitize_identifier(self, name: str) -> str:
        """Converts invalid name -> valid name (e.g. 'My Table' -> 'MY_TABLE')."""
        pass

    # --- Section B: Metadata Value Formatting ---

    @abstractmethod
    def format_date_literal(self, iso_date_str: str) -> str:
        """
        Converts generic 'YYYY-MM-DD' to system specific string.
        Input: '2023-12-31'
        Output (Snowflake): "'2023-12-31'"
        Output (Fabric): "DATE(2023, 12, 31)"
        """
        pass

    @abstractmethod
    def format_boolean_literal(self, value: bool) -> str:
        """
        Converts uv run bool to system string.
        Output (Python): True
        Output (SQL): 'TRUE' or '1'
        output (DAX): TRUE()
        """
        pass

    @abstractmethod
    def map_data_type(self, sml_type: str) -> str:
        """
        Maps SML types to Target Metadata types.
        Input: 'string' -> Output: 'VARCHAR(16777216)' (Snowflake)
        Input: 'string' -> Output: 'string' (Power BI)
        """
        pass`

### 3. Validation & Conversion Flow

---

When theÂ `SmlToTargetConverter`Â processes a model, it must use these functions to transformÂ **values**Â found in the metadata (like default values, partition keys, or security filters).

**Example Workflow:**

1. **Input (SML):**Â A measure has a description with a default filter:Â `date > 2023-01-01`.
2. **Lookup:**Â Converter callsÂ `target_fmt.format_date_literal('2023-01-01')`.
3. **Transformation:**
    - If Target isÂ **Databricks**: Result isÂ `DATE '2023-01-01'`.
    - If Target isÂ **Fabric**: Result isÂ `DATE(2023, 1, 1)`.
4. **Code Generation:**Â The result is injected into the final uv run code and the sync is completed successfully.

## 4. Developer Documentation: Adding a Format

1. **Create File:**Â `src/semabridge/formats/new_system_fmt.py`
2. **Define Regex:**Â Add theÂ `VALID_PATTERN`Â for identifiers.
3. **Define Literals:**Â ImplementÂ `format_date_literal`Â and others.Â **Crucial:**Â Check how the target system accepts hardcoded values in its DDL/Scripting language.
4. **Define Types:**Â Map the 5 SML primitive types to the target's native types.
5. **Test:**Â Add unit tests inÂ `tests/formats/test_new_system.py`Â passing dirty strings and checking sanitized outputs.

