...
```
BEFORE (Unreliable):
  metric_name â†’ guess columns/aggregation â†’ LLM inference
  
AFTER (Deterministic):
  DAX definition â†’ parse structure â†’ resolve columns â†’ SQL
```
...
```uv run # Create pipeline with schema columns
...
```uv run # Get DAX from Power BI model
...
3. **Deterministic**: Same input â†’ always same output
...
OLD (Name)    | 30%      | 300K     | ~Â£600
NEW (DAX)     | 5%       | 50K      | ~Â£100
NEW (Optimal) | <1%      | 10K      | ~Â£20
```
...
```uv run pipeline.add_column_mapping('CostAmount', 'COST_AMOUNT')
...
```uv run print(f"Schema: {pipeline.dictionary.schema_columns}")
...
# Common reasons:
# - RANKX/EARLIER/PRIOR â†’ Use LLM
# - Unsupported aggregation â†’ Update generator
# - Missing column mapping â†’ Add mapping
```
...
```uv run def test_simple_aggregation():
...
```uv run def test_with_real_model():
