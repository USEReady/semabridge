...
### 2.1 OSI (Open Semantic Interchange)
 The canonical intermediate format that abstracts source-specific metadata (TMDL, PBIX) into a platform-agnostic schema.
...
- **Fabric/PBIX Extractor**: Uses TMDL (Tabular Model Definition Language) to pull model definitions.
...
### 7.1 Sync Pipeline
1. **Extract**: Pull source model (TMDL).
2. **Convert**: TMDL → OSI.
