...
# 1. Inject the hook in _execute_deployment_pipeline
hook_code = '''                # Store model for helper methods
                self._model = model
...
target_hook_line = '                cur = conn.cursor()'
replacement_hook = target_hook_line + '\n' + hook_code
if target_hook_line in content:
    content = content.replace(target_hook_line, replacement_hook, 1)

# 2. Append the 5 methods
methods_code = '''

    def _identify_fact_table(self, model: Any) -> Optional[str]:
        """
    ...
    def _create_enriched_view(self, model: Any, cursor: Any) -> Optional[str]:
        """
        ...
        date_info = getattr(self, '_find_date_table', lambda m: None)(model)
...
if '_identify_fact_table' not in content:
