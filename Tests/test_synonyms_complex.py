...
class TestComplexScenarios:
    ...
    def test_homoglyph_synonym_attack(self):
        ...
        # Latin 'A' vs Cyrillic 'Ð' (visually identical but different Unicode)
        latin_a = "A"
    ...
    @pytest.mark.parametrize("multibyte", [
        "ðŸ’° Profit",  # Emoji
        "é”€å”®æ”¶å…¥",     # Chinese
        "Ø§Ù„Ø¥ÙŠØ±Ø§Ø¯Ø§Øª",   # Arabic
        "ðŸ“Š Revenue ðŸ“ˆ", # Mixed
        "MÃ¼nchen ðŸº",   # Latin + Emoji
        "à¤¨à¤«à¤¾",         # Hindi
    ])
    ...
    def test_extreme_nesting_thousand_columns(self):
        """Performance test: 1000 columns Ã— 5 synonyms each"""
        import time
    ...
    @pytest.mark.parametrize("symbol", [
        "âˆ‘", "Î”", "Ï€", "âˆš", "âˆž", "â‰ ", "â‰¤", "â‰¥", "â€ ", "â€¡", "â†’", "â†", "â†‘", "â†“"
    ])
    ...
    def test_utf16_vs_utf8_confusion(self):
        ...
        result = merge_synonyms([utf8_syn], ["CafÃ©"])
        assert len(result) == 1
    ...
    def test_mixed_ltr_rtl_synonyms(self):
        """LTR and RTL mix"""
        clause = synonyms_clause(["Revenue", "Ø§Ù„Ø¥ÙŠØ±Ø§Ø¯Ø§Øª"])
        assert "Revenue" in clause and "Ø§Ù„Ø¥ÙŠØ±Ø§Ø¯Ø§Øª" in clause
