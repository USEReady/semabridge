...
```
Revenue % Growth  → REVENUE_GROWTH    (% and space replaced)
customer-id       → CUSTOMER_ID       (hyphen replaced)
sales@2024        → SALES_2024        (@ replaced)
price$            → PRICE$            ($ preserved)
```
...
```
Revenue%           → REVENUE   (% → _ → stripped)
 Revenue           → REVENUE   (spaces stripped)
```
...
```
date (first occurrence)  → COL_DATE
DATE (different source)  → COL_DATE_2
Date (yet another source) → COL_DATE_3
```
...
_sanitize_identifier("2024 Sales")  # → "_2024_SALES"
_sanitize_identifier("Revenue %")   # → "REVENUE"
```
...
**Handles:**
- DAX table qualifiers: `'Sales'[Amount]` → `Amount`
- Dot notation: `SALES.AMOUNT` → `AMOUNT`
- Digit prefixing
...
```python
result = sanitizer.sanitize_table_name("2024-Sales_Report")
# → "_2024_SALES_REPORT"
```

#### Method 3: `sanitize_alias(name)` - WITH RESERVED WORD HANDLING
```python
sanitizer_with_reserved = IdentifierSanitizer(suppress_reserved=True)
...
name1 = registry.register("Revenue %")   # → "REVENUE"
name2 = registry.register("Revenue @")   # → "REVENUE_2" (collision detected)
name3 = registry.register("Revenue #")   # → "REVENUE_3"

# Get collision summary
summary = registry.get_collision_summary()
# → {"REVENUE": 1}  (1 collision = 2 occurrences)
...
for name in names:
    ...
    print(f"{name:40} → {sanitized}")

# Output:
# 18_MONTH_FORWARD_LOOKING_PIPELINE_MWDC  → _18_MONTH_FORWARD_LOOKING_PIPELINE_MWDC
# Revenue % Growth                    → REVENUE_GROWTH
# customer-id                        → CUSTOMER_ID
# 2024 Sales                        → _2024_SALES
# sales@2024#report                 → SALES_2024_REPORT
```

### Example 2: Fabric Model Extract → Snowflake Deploy
```python
from semabridge.utils.identifiers import IdentifierRegistry
...
for col in fabric_columns:
    ...
    print(f"Column: {col:30} → {normalized}")

# Log summary for audit
registry.log_summary()

# Output:
# Column: Revenue % Growth          → REVENUE_GROWTH
# Column: 2024 Sales Amount        → _2024_SALES_AMOUNT
...
collision_sources = [
    ...
    "Revenue @",   # This also → "REVENUE" (collision!)
]

for source in collision_sources:
    normalized = registry.register(source, name_type="column")
    print(f"{source:20} → {normalized}")

# Output:
# Revenue %            → REVENUE     (first occurrence)
# Revenue $            → REVENUE_$   ($ preserved, different name)
# Revenue @            → REVENUE_2   (collision with Revenue %)
```
...
for keyword in reserved_keywords:
    ...
    print(f"{keyword:20} → {normalized}")

# Output:
# date                 → COL_DATE
# DATE                 → COL_DATE_2   (collision with 'date')
# customer             → CUSTOMER
# select               → COL_SELECT
...
```python
# In snowflake_emitter.py __init__:
...
sanitized_col = self._sanitize_col_name(raw_col_name)  # → calls self._id.sanitize_column()
sanitized_table = self._sanitize_table_name(raw_table) # → calls self._id.sanitize_table_name()
```
...
```
DEBUG: sanitize_column: 'Revenue' → 'REVENUE'
DEBUG: sanitize_table_name: 'customer_table' → 'CUSTOMER_TABLE'
DEBUG: sanitize_alias: 'sort' → 'SORT'
```

### Info Level (logger.info)
```
INFO: column: 'Revenue %' → 'REVENUE'
INFO: column: '2024 Sales' → '2024 Sales' (collision) → '_2024_SALES_2'
...
```
WARNING: Identifier sanitization: 'invalid###name' → 'INVALID___NAME' contains unexpected characters after normalization
```
...
```python
collisions = registry.get_collision_summary()
if collisions:
...
```
GOOD:   "Revenue % Growth" → "REVENUE_GROWTH"
  ✓ Preserves "Revenue" and "Growth"
  
AVOID:  "Revenue % Growth" → "R_G"
  ✗ Loses semantic meaning (abbreviation is wrong)
```
...
```
"price$amount"  → "PRICE$AMOUNT"  (valid)
"Price%Amount"  → "PRICE_AMOUNT"  (% replaced)
```
