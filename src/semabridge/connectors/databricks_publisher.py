...
class DatabricksPublisher:
    ...
    def publish(self, sml_model: SMLModel) -> str:
        ...
        retry_mgr = RetryManager(
            config=RetryConfig(
                max_retries=3, 
                base_delay=0.0 if is_test else 0.5, 
                max_delay=0.0 if is_test else 10.0
            ),
        ...
        if (
            ...
            def _deploy_fallback_view(view_sql: str) -> tuple[str, bool, str]:
                """Deploy a single SQL fallback view with retry."""
                view_name = "unknown"
                match = re.search(r'VIEW\s+(`[^`]+`\.`[^`]+`\.`[^`]+`)', view_sql)
                if match:
                    view_name = match.group(1)
                try:
                    retry_mgr.execute_with_retry(
                        operation=lambda: self.execute_statements([view_sql]),
                        operation_name=f"fallback_view:{view_name}",
                    )
                    return view_name, True, ""
                except Exception as exc:
                    return view_name, False, str(exc)[:200]

            if fallback_view_stmts:
                fb_workers = min(MAX_VIEW_WORKERS, len(fallback_view_stmts))
                with ThreadPoolExecutor(max_workers=fb_workers) as pool:
                    futures = {
                        pool.submit(_deploy_fallback_view, sql): sql
                        for sql in fallback_view_stmts
                    }
                    for future in as_completed(futures):
                        view_name, success, error_msg = future.result()
                        if success:
                            ...
                            fallback_failed += 1
                            logger.warning(
                                "⚠️  SQL fallback view %s failed: %s",
                                view_name, error_msg,
                            )
