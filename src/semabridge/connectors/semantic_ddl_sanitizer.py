...
class SemanticDDLSanitizer:
    ...
    def format_physical_column_ref(
        ...
        *,
        ...
        # Guard: if phys_col was resolved as a dotted reference (e.g. "COL_DATE.YEAR"
        # from a Calendar.Year source expression), strip the table qualifier so we
        # emit alias."YEAR" rather than alias."COL_DATE.YEAR" which Snowflake rejects.
        if phys_col and "." in phys_col and not phys_col.startswith('"'):
            parts = phys_col.split(".", 1)
            phys_col = parts[-1]
